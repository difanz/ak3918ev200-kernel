 ...
 */
 struct akotg_dma
 {
  ...
  struct akotg_usbhc {
	 ...
	 struct gpio_desc	*pwr_on_gpiod;
	 struct gpio_desc	*pwr_off_gpiod;
	 struct gpio_desc	*sw_onboard_gpiod;
	 struct gpio_desc	*sw_extport_gpiod;
	 ...
	 u32		 port_status;
	 /* Asserting POWER_RESET makes this MUSB core raise a DISCONNECT it did
	  * not mean; ignore_disconnect below suppresses that spurious event. */
...
/*
 * L2 buffer id table for USB EPFIFO 2..MAX_EP_NUM. The four USB endpoint
 * buffers sit at the same four L2 bit positions {0, 1, 2, 15}.
 */
static inline u8 akotg_alloc_l2_buffer(int epfifo)
{
	l2_device_t l2addr[] = {ADDR_USB_EP1, ADDR_USB_EP2, ADDR_USB_EP3, ADDR_USB_EP4};
