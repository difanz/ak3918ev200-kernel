...
typedef struct ak_isp_3d_nr {
	...
	T_U16		tnr_refFrame_format;	//参考帧的格式
/*
 * Added after this unit's firmware was built, and ak39_isp2.c already
 * excludes the matching register write for EV200. Keeping the field makes
 * sizeof(AK_ISP_3D_NR_ATTR) 1124 while the stock conf module carries 1084,
 * so the driver's copy_from_user reads 40 bytes past the caller's object -
 * silently, into whatever follows it.
 */
