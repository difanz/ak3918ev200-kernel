...
typedef struct ak_isp_blc {
    T_U16     black_level_enable;	//enable bit
    T_U16     bl_r_a;				//[ 0,1023]
...
typedef struct ak_isp_blc_attr {
    T_U16       blc_mode;             //0: linked mode, 1: manual mode
    AK_ISP_BLC  m_blc;
...
typedef struct ak_isp_rgb_gamma_attr {
    ...
    T_U16   g_key[16];
    T_U16   b_key[16];
    T_U16   rgb_gamma_enable;  //when disabled, the curve is a straight line
} AK_ISP_RGB_GAMMA_ATTR;

typedef struct ak_isp_y_gamma_attr {
	T_U16    ygamma[129];    //10bit
	T_U16    ygamma_key[16]; //curve control points
	T_U16    ygamma_uv_adjust_enable;
	T_U16    ygamma_uv_adjust_level;
	T_U16    ygamma_cnoise_yth1;   //Ygamma chroma-noise suppression threshold
	T_U16    ygamma_cnoise_yth2;   //Ygamma chroma-noise suppression threshold
	T_U16    ygamma_cnoise_slop;   
	T_U16    ygamma_cnoise_gain ;  //UV adjustment coefficient calculation parameter
}AK_ISP_Y_GAMMA_ATTR;

typedef struct ak_isp_nr1 {
    T_U16  nr1_enable;            //enable bit
...
typedef struct ak_isp_s_nr1_attr {
    T_U16       nr1_mode;             //nr1 mode, auto or linked
    AK_ISP_NR1  manual_nr1;
    AK_ISP_NR1  linkage_nr1[9];       //linked-mode parameters
}AK_ISP_NR1_ATTR;
...
typedef struct ak_isp_nr2_attr {
    T_U16       nr2_mode;          //manual or linked mode
...
typedef struct  ak_isp_3d_nr {
    ...
    T_U16 MD_th;              	//motion detection threshold [0-127]
    T_U16 updata_ref_y;       	//update Y
    T_U16 updata_ref_uv;		//update UV
    T_U16 tnr_refFrame_format;	//reference frame format
...
typedef struct ak_isp_3d_nr {
	...
	T_U16		tnr_refFrame_format;	//reference frame format
/*
 * This field was added to the SDK after the userspace config tool was
 * built, and ak39_isp2.c already excludes the matching register write for
 * EV200. Keeping the field makes sizeof(AK_ISP_3D_NR_ATTR) 1124 while the
 * userspace tool's structure is 1084 bytes, so the driver's copy_from_user
 * reads 40 bytes past the caller's object - silently, into whatever
 * follows it.
 */
	...
	T_U32		md_th;			//[0, 65535]  motion detection threshold [0-127]
}AK_ISP_3D_NR;
...
typedef struct  ak_isp_3d_nr_stat_info {
	T_U16		MD_stat_max;	
	T_U16		MD_stat[24][32];		//motion detection zoned output
	T_U32		MD_level;			    //motion detection output
}AK_ISP_3D_NR_STAT_INFO;

typedef struct ak_isp_gb {

    T_U16   gb_enable;        //enable bit
...
typedef struct ak_isp_gb_attr {
    T_U16      gb_mode;       //mode select, manual or linked
    AK_ISP_GB  manual_gb;
    AK_ISP_GB  linkage_gb[9];
} AK_ISP_GB_ATTR;

typedef struct ak_isp_demo_attr {
    T_U16  dm_HV_th;       //direction discrimination coefficient
...
typedef struct ak_isp_ccm {
	T_U16  cc_enable;		   //color correction enable
	T_U16  cc_cnoise_yth;	   //luminance control gain
	T_U16  cc_cnoise_gain;	   //luminance control gain
	T_U16  cc_cnoise_slop;	   //luminance control gain
	T_S16  ccm[3][3];		   //[-2048, 2047]    
}AK_ISP_CCM;

typedef struct ak_isp_ccm_attr {
    T_U16       cc_mode;  		//color correction matrix, linked or manual
    AK_ISP_CCM  manual_ccm; 
    AK_ISP_CCM  ccm[4]; 		//four linked matrices
}AK_ISP_CCM_ATTR;

typedef struct ak_isp_wdr {
    T_U16   hdr_uv_adjust_level;        //UV adjustment level, [0,31]
    T_U16   hdr_cnoise_suppress_slop;   //suppression slope
    T_U16   wdr_enable;
	...
	T_U16	area_tb1[65];	  //curve, 10-bit
	T_U16	area_tb2[65];	  //curve, 10-bit
	T_U16	area_tb3[65];	  //curve, 10-bit
	T_U16	area_tb4[65];	  //curve, 10-bit
	T_U16	area_tb5[65];	  //curve, 10-bit
	T_U16	area_tb6[65];	  //curve, 10-bit
    ...
    T_U16   hdr_uv_adjust_enable;       //UV adjustment enable
    T_U16   hdr_cnoise_suppress_yth1;   //color-noise luminance threshold 1
    T_U16   hdr_cnoise_suppress_yth2;   //color-noise luminance threshold 2
    T_U16   hdr_cnoise_suppress_gain;   //chroma suppression
}AK_ISP_WDR;

typedef struct ak_isp_wdr_attr {
	T_U16 wdr_mode;              //mode select, manual or linked
...
typedef struct  ak_isp_contrast_ATTR {
	T_U16	cc_mode;	//mode select, manual or linked
...
typedef  struct ak_isp_fcs {
    ...
    T_U16  fcs_enable;   		//enable bit
    T_U16  fcs_uv_nr_enable;  	//enable bit
    T_U16  fcs_uv_nr_th;  		//[0, 1023]
}AK_ISP_FCS;

typedef  struct ak_isp_fcs_attr {
    T_U16       fcs_mode;   	//mode select, manual or linked
    AK_ISP_FCS  manual_fcs;
    AK_ISP_FCS  linkage_fcs[9];
}AK_ISP_FCS_ATTR;

typedef struct ak_isp_hue {
	T_U16  hue_sat_en;			//hue enable
...
typedef struct ak_isp_hue_attr {
	T_U16        hue_mode;		//linked or manual
	AK_ISP_HUE   manual_hue; 
	AK_ISP_HUE   hue[4];		//four linked parameters
}AK_ISP_HUE_ATTR;

typedef  struct ak_isp_rgb2yuv_attr {
    T_U16  mode; 				//bt601 or bt709
}AK_ISP_RGB2YUV_ATTR;
...
typedef struct ak_isp_effect_attr {
    ...
    T_S16  uv_a;    //[-256, 255]
    T_S16  uv_b;    //[-256, 255]
    T_U16  dark_margin_en;   //black-margin enable
}AK_ISP_EFFECT_ATTR;

typedef struct ak_isp_ddpc { 
	T_U16 ddpc_enable;         //dynamic defect-pixel-correction enable bit
	T_U16 ddpc_th;             //10bit
	T_U16 white_dpc_enable;    //white-defect-pixel removal enable bit
	T_U16 black_dpc_enable;    //black-defect-pixel removal enable bit
}AK_ISP_DDPC;
 
typedef struct ak_isp_ddpc_attr {
	T_U16 ddpc_mode;            //mode select, manual or linked
	AK_ISP_DDPC manual_ddpc;
	AK_ISP_DDPC linkage_ddpc[9]; 
}AK_ISP_DDPC_ATTR;

typedef struct ak_isp_sdpc_attr { 
    T_U32   sdpc_enable; 		//static defect-pixel-correction enable bit

    /* Static defect-pixel coordinates, up to 1024 entries.
    Format: {6h0 ,y_position[9:0],5'h0,x_position[10:0]} */
...
typedef struct ak_isp_af_stat_info {
   T_U32  af_statics[5];            //statistics result
}AK_ISP_AF_STAT_INFO;

typedef struct ak_isp_weight_attr {
   T_U16   zone_weight[8][16];      //weight coefficients
}AK_ISP_WEIGHT_ATTR;
...
typedef  struct ak_isp_awb_attr {   
    ...
    //parameters the AWB software needs to configure
    T_U16   auto_wb_step;                 //white balance step calculation
    T_U16   total_cnt_thresh;            //pixel count threshold
    T_U16   colortemp_stable_cnt_thresh; //stable frame count; number of consecutive matching frames before the ambient color temperature is considered changed
...
typedef  struct ak_isp_awb_stat_info {
    //white balance statistics result
    T_U32   total_R[10];
    T_U32   total_G[10];
    T_U32   total_B[10];
    T_U32   total_cnt[10];
    //white balance gain computed by the auto white balance algorithm
    T_U16   r_gain;
    T_U16   g_gain;
    T_U16   b_gain;
    T_S16   r_offset;
    T_S16   g_offset;
    T_S16   b_offset;
    T_U16   current_colortemp_index;     //ambient color-temperature index; a color-temperature indicator that varies with the environment.
    T_U16   colortemp_stable_cnt[10];         //stable frame count for each color temperature
...
typedef struct ak_isp_ae_attr {
    T_U32   exp_time_max;			//max exposure time
    T_U32   exp_time_min;			//min exposure time
    T_U32   d_gain_max;      		//max digital gain
    T_U32   d_gain_min;     		//min digital gain
    T_U32   isp_d_gain_min;  		//min ISP digital gain
    T_U32   isp_d_gain_max;  		//max ISP digital gain
    T_U32   a_gain_max;     		//max analog gain
    T_U32   a_gain_min;      		//min analog gain
    T_U32   exp_step;            	//user exposure adjustment step
    T_U32   exp_stable_range;     	//stable range
    T_U32   target_lumiance;     	//target luminance
...
typedef  struct ak_isp_ae_run_info {
    T_U8   current_calc_avg_lumi;         		//currently computed luminance value
    T_U8   current_calc_avg_compensation_lumi; 	//luminance value after exposure compensation

    T_U8   current_darked_flag;                 //day/night flag

    T_S32  current_a_gain;                     	//analog gain value
    T_S32  current_d_gain;                    	//digital gain value
    T_S32  current_isp_d_gain;                	//ISP digital gain value
    T_S32  current_exp_time;                	//exposure time value

    T_U32  current_a_gain_step;             	//current analog gain step
    T_U32  current_d_gain_step;             	//digital gain step
    T_U32  current_isp_d_gain_step;        		//ISP digital gain step
    T_U32  current_exp_time_step;          		//exposure time step
}AK_ISP_AE_RUN_INFO;
    
typedef struct ak_isp_aec_algo {
    T_U32  aec_status;       		//AEC status, stable or unstable
    T_U32  aec_locked;      		//whether AEC is locked
    
    T_U8  exp_time_need_updata;   	//needs update; reduces I2C read/write count
    T_U8  a_gain_need_updata;    	//needs update; reduces I2C read/write count
    T_U8  d_gain_need_updata;       //needs update; written only when an update is needed
    T_U8  isp_d_gain_need_updata; 	//needs update
    T_U8  lock_range;
}AK_ISP_AEC_ALGO;
...
typedef struct ak_isp_misc_attr {
	...
	T_U16  mirror_en;
/*
 * Same again: sizeof(AK_ISP_MISC_ATTR) is 34 with these and 26 without, and
 * the stock conf module is 26. This part takes a parallel sensor, not MIPI,
 * and ak39_isp2.c already excludes both register writes for EV200.
 */
#ifndef CONFIG_CPU_AK3918EV200
...
typedef struct ak_isp_mask_color_attr {
    T_U8  color_type;   //0: specify mask color; 1:
    T_U8  mk_alpha;
    T_U8  y_mk_color;
    T_U8  u_mk_color;
    T_U8  v_mk_color;
...
/* Non-zero once the ISP core has been brought up by opening /dev/isp_char.
 * Everything below dereferences that state without checking it. */
