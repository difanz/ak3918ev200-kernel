 ...
 * @return void
 */
static int snd_card_akpcm_pcm(struct snd_akpcm *akpcm, int device,
					int substreams)
{
	struct snd_pcm *pcm;
	struct snd_pcm_substream *substream;
 ...
 * @retval    <0 :  failed
 */
static int snd_akpcm_probe(struct platform_device *devptr)
{
	struct snd_card *card;
	struct snd_akpcm *akpcm;
	//struct snd_info_entry *entry;
	...
	if (dev_id < 0)
		dev_id = 0;
	err = snd_card_new(&devptr->dev, dev_id, NULL, THIS_MODULE,
			    sizeof(struct snd_akpcm), &card);
	if (err < 0)
		return err;
 ...
 * @retval    <0 :  failed
 */
static int snd_akpcm_remove(struct platform_device *devptr)
{
 ...
 * @return void
 */
static u64 snd_akpcm_dma_mask = DMA_BIT_MASK(32);

static struct platform_device snd_akpcm_device = {
	.name	= SND_AKPCM_DRIVER,
	.id	= 0,
	.dev	= {
		.dma_mask	   = &snd_akpcm_dma_mask,
...
static int __init alsa_card_akpcm_init(void)
{
	...
	err = platform_device_register(&snd_akpcm_device);
	if (err < 0)
		return err;

	err = platform_driver_register(&snd_akpcm_driver);
	if (err < 0) {
		platform_device_unregister(&snd_akpcm_device);
		return err;
	}
