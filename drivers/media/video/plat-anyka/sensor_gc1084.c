 ...
 *
 * Written against the register programming recovered from this camera's own
 * vendor files; see notes/gc1084-sensor.md. The initialisation script is not
 * here - the ISP loads it from isp_gc1084.conf and hands it to
 * gc1084_init() - so this driver only carries what the vendor .ko carried:
 * the gain ladder, the exposure and frame-rate arithmetic, and the accessors.
 */
 ...
 * is what the neighbouring Anyka sensor drivers declare. Verify on hardware. */
 ...
 * +0x58, regValTable at +0xc0) - see notes/gc1084-sensor.md, where the values
 * are re-derived from the binary rather than taken on trust.
 */
