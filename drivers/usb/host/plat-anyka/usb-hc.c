...
void port_power(struct akotg_usbhc *akotghc, int is_on)
{
	...
	if (is_on) {
		if (akotghc->port_status & (1 << USB_PORT_FEAT_POWER) && HC_IS_RUNNING(hcd->state))
			return;
        ...
        */
        ...
        if((len == 512) && (remain >= len))
        {
            ...
            do
            {
                ...
                phyaddr = urb->transfer_dma + urb->actual_length;
                if (phyaddr == 0) {
                    pr_err("rx phyaddr error!\n");
                    break;
                }
...
done(struct akotg_usbhc *akotghc, struct akotghc_ep *ep)
{
	...
	if (err_occurred) {
		/* TXCSR1 is 0x12 and TXCSR2 is 0x13, so a 16-bit store here also
		 * zeroes TXCSR2 - and with it TXCSR2_MODE, which set_epx_tx_mode()
		 * writes exactly once when the endpoint is first mapped. Losing it
		 * turns a shared FIFO back to RX and the endpoint transmits nothing
		 * ever after. Byte writes, and re-assert the direction. */
		if (epfifo == 0) {
			hc_index_writeb(0, 0, USB_REG_TXCSR1);
		} else if (is_out) {
			hc_index_writeb(epfifo, (urbstat == -EPIPE) ? (1 << 6) : 0,
					USB_REG_TXCSR1);
			set_epx_tx_mode(epfifo);
		} else {
			hc_index_writeb(epfifo, (urbstat == -EPIPE) ? (1 << 7) : 0,
					USB_REG_RXCSR1);
...
irqreturn_t akotg_usbhc_irq(struct usb_hcd *hcd)
{
	...
	if ((rINTCOM & USB_INTR_DISCONNECT) && akotghc->ignore_disconnect) {
		VDBG("Ignoring the disconnect our own port reset caused\n");
		rINTCOM &= ~USB_INTR_DISCONNECT;
	}
...
akotg_usbhc_endpoint_disable(struct usb_hcd *hcd, struct usb_host_endpoint *hep)
{
	...
	/* epfifo is still the initialiser here, so this used to disable EP0's
	 * interrupt and flush EP0's FIFO whenever any endpoint was torn down -
	 * undoing enable_ep0_interrupt(). Look the endpoint up. */
	if (epnum_to_epfifo(&akotg_epfifo_mapping, epnum, is_out, &epfifo)) {
