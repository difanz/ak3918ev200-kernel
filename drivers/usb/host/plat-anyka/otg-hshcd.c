...
static void usb_poweron_for_device(struct akotg_usbhc *akotghc)
{
	/* power on for usb device */
	if (akotghc->pwr_on_gpiod)
		gpiod_set_value_cansleep(akotghc->pwr_on_gpiod, 1);

	if (akotghc->sw_onboard_gpiod && akotghc->sw_extport_gpiod) {
		if (!strcmp(host_sw, "extport"))
			gpiod_set_value_cansleep(akotghc->sw_extport_gpiod, 1);
		else
			gpiod_set_value_cansleep(akotghc->sw_onboard_gpiod, 1);
	}
}

static void usb_poweroff_for_device(struct akotg_usbhc *akotghc)
{
	if (akotghc->sw_onboard_gpiod && akotghc->sw_extport_gpiod) {
		if (!strcmp(host_sw, "onboard"))
			gpiod_set_value_cansleep(akotghc->sw_extport_gpiod, 0);
		else
			gpiod_set_value_cansleep(akotghc->sw_onboard_gpiod, 0);
	}

	/* power off for usb device */
	if (akotghc->pwr_off_gpiod)
		gpiod_set_value_cansleep(akotghc->pwr_off_gpiod, 1);
}
...
akotg_hc_remove(struct platform_device *pdev)
{
	...
	usb_poweroff_for_device(akotghc);
...
akotg_hc_probe(struct platform_device *pdev)
{
	...
	akotghc->pwr_on_gpiod = devm_gpiod_get_optional(&pdev->dev, "pwr-on", GPIOD_OUT_LOW);
	akotghc->pwr_off_gpiod = devm_gpiod_get_optional(&pdev->dev, "pwr-off", GPIOD_OUT_LOW);
	akotghc->sw_onboard_gpiod = devm_gpiod_get_optional(&pdev->dev, "sw-onboard", GPIOD_OUT_LOW);
	akotghc->sw_extport_gpiod = devm_gpiod_get_optional(&pdev->dev, "sw-extport", GPIOD_OUT_LOW);

	usb_poweron_for_device(akotghc);

	akotghc->clk = clk_get(&pdev->dev, NULL);
	if (IS_ERR(akotghc->clk)) {
		dev_dbg(&pdev->dev, "usb otg hs clocks missing\n");
		akotghc->clk = NULL;
		goto err_nomem;
	}
...
static const struct of_device_id akotg_hc_of_match[] = {
	{ .compatible = "anyka,ak39ev330-usb" },
	{ }
};
MODULE_DEVICE_TABLE(of, akotg_hc_of_match);
...
struct platform_driver akotg_hc_driver = {
	...
	.driver = {
		.name =	(char *) hcd_name,
		.owner = THIS_MODULE,
		.of_match_table = akotg_hc_of_match,
	},
};
