...
/*
 * sharepin caveats:
 * 1) e.g. the SD1 interface can be configured for 1-wire or 4-wire mode,
 * 		and SD1 can select either of two pin groups for output.
 * 		Must ensure: e.g. in 1-wire mode, D3 is used for another function,
 * 		but this must not affect SD1 output on either group.
 * 2) For manageability, some pins' function assignment differs from PG.
 * */

/*
{
	Rough sharepin configuration flow:
	if (reg0)
		clear reg0_mask;
	if (reg1)
		clear reg1_mask;
	...
	if (func == func0) {
		clear func0_reg0_mask;
		set func0_reg0_value;
	}
	if (func == func1) {
		clear func0_reg1_mask;
		set func0_reg1_value;
	}
	if (func == func2) {
		clear func0_reg2_mask;
		set func0_reg2_value;
	}
}
*/
...
static struct gpio_sharepin ak_sharepin[] = {
	...
	/*Note: differs from PG here; gpio and ISP_PWM0 are split into two functions*/
	...
	/*Note: differs from PG here; gpio and ISP_PWM1 are split into two functions*/
	...
	/*The two I2S pin groups don't cross; configure extern reg at LRCLK*/
	/*The two SPI1 pin groups don't cross; configure extern reg at CLK*/
	/*I2S set extern reg*/
	...
	/*SD1's two pin groups share the same clk and cmd; can only distinguish via D0*/
	...
	{24, AK_SHAREPIN_CON2, 0x3<<24, NULL, 0, NULL, 0,
		...
		AK_PULL_REG2, 16, 0},
	/* gpio, SPI0_CS*/
	/*SPI0 only has cs and sclk fixed at gpio[26:25]; the rest split into two groups:
	 *	(1)[40:37]
	 *	(2)[79],[36:34]*/
	...
	/*SD1's two groups share the same clk, so configure extern reg at D0*/
	/*SD1 extern reg*/
...
{
	/*H3B: only the mipi pins have selectable pull-up/down; other pins have only one option*/
...
static int pin_get_pull_polarity(struct ak_pinctrl *pc, int pin)
{
	...
	if (ak_sharepin[pin].pupd_en_reg) {
		/*If a pin supports pull-up and pull-enable is set, treat it as pull-up polarity; otherwise pull-down*/
