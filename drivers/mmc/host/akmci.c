...
static inline void akmci_drv_lock(struct akmci_host *host) { }
static inline void akmci_drv_unlock(struct akmci_host *host) { }


#define MCI_L2_ADDR(host)	\
	((host->mci_mode == MCI_MODE_MMC_SD) ? ADDR_MMC0 : ADDR_MMC1)
...
static int akmci_card_present(struct mmc_host *mmc)
{
	...
	if(host->detect_mode == AKMCI_DETECT_MODE_GPIO)
	{
		...
		if(host->gpio_cd >= 0)
			return (gpio_get_value(host->gpio_cd) == 0);
		else
			return 1;
	}
	else
...
static int akmci_init_mmc_host(struct akmci_host *host)
{
	...
	mmc->caps = (plat->data_lines == 1) ? 0 : MMC_CAP_4_BIT_DATA;
...
static int akmci_probe(struct platform_device *pdev)
{
	...
	plat->mci_mode = (int)(uintptr_t)match->data;

	plat->data_lines = 4;
	if (!of_property_read_u32(np, "bus-width", &val))
		plat->data_lines = val;
	plat->cap_highspeed = of_property_read_bool(np, "cap-sd-highspeed") ||
			      of_property_read_bool(np, "cap-mmc-highspeed");

	plat->max_speed_hz = 25 * 1000 * 1000;
	if (!of_property_read_u32(np, "max-frequency", &val))
		plat->max_speed_hz = val;

	if (of_find_property(np, "cd-gpios", NULL))
		plat->detect_mode = AKMCI_DETECT_MODE_GPIO;
	else
		plat->detect_mode = AKMCI_PLUGIN_ALWAY;
	plat->irq_cd_type = IRQ_TYPE_LEVEL_LOW;

	plat->xfer_mode = AKMCI_XFER_L2DMA;
	...
	if (request_irq(host->irq_mci, akmci_irq, 0, pdev->name, host)) {
	...
	if(host->detect_mode == AKMCI_DETECT_MODE_GPIO) {
		int gpio = of_get_named_gpio(np, "cd-gpios", 0);

		if (gpio_is_valid(gpio) &&
		    !devm_gpio_request_one(&pdev->dev, gpio, GPIOF_IN, "mci-cd")) {
	    	host->gpio_cd = gpio;
	        ...
	        ret = request_irq(host->irq_cd, akmci_card_detect_irq,
					IRQF_TRIGGER_LOW, pdev->name, host);
	...
	{
		int gpio = of_get_named_gpio(np, "wp-gpios", 0);
...
static const struct of_device_id akmci_of_match[] = {
	{ .compatible = "anyka,ak39ev330-mmc0", .data = (void *)MCI_MODE_MMC_SD },
	{ .compatible = "anyka,ak39ev330-mmc1", .data = (void *)MCI_MODE_SDIO },
	{ },
};
